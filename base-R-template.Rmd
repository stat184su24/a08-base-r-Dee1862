...
```{r}
#Stores all the numbers from 1 to 10 in num.
num <- 1:10

#Running a for loop.
for (values in num) {
  #Condition to multiply all the numbers less than 5 with 3 and print them.
  if (values >= 5) {
    print(values * 2)
  }else { #Condition to multiply all the numbers greater than or equal to 5 with 2 and print them.
    print(values * 3)
  }
}
...
```{r}
# V1, V2, V3, and V4 consists of all the numbers from 1 to 3.
V1 <- 1:3
V2 <- 1:3
V3 <- 1:3
V4 <- 1:3

#This loop creates 3 rows and 4 columns.
for (r in 1:3) {
  for (c in 1:4) {
    if (c == 1) {
      V1[r] <- r + 0
...
#A new variable is added to the function
